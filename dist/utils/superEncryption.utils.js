 ...
 */
function loadKeyFromEnv(envName) {
    ...
    if (!v) {
        ...
        if (envName === "CHACHA_KEY")
            return Buffer.from("fedcba9876543210fedcba9876543210", "hex");
...
function superEncrypt(plainText) {
    ...
    const aes = crypto_1.default.createCipheriv("aes-256-gcm", AES_KEY, ivAes);
    ...
    // ChaCha20-Poly1305
    const ivCha = crypto_1.default.randomBytes(12);
    const chacha = crypto_1.default.createCipheriv("chacha20-poly1305", CHACHA_KEY, ivCha, { authTagLength: 16 });
...
function superDecrypt(pkg) {
    ...
    const chacha = crypto_1.default.createDecipheriv("chacha20-poly1305", CHACHA_KEY, ivCha, { authTagLength: 16 });
    ...
    if (inner.length < 28)
        throw new Error("Inner decrypted too short");
    const innerIvAes = inner.slice(0, 12);
    const innerTagAes = inner.slice(12, 28);
    const ctAes = inner.slice(28);
    const decipher = crypto_1.default.createDecipheriv("aes-256-gcm", AES_KEY, innerIvAes);
