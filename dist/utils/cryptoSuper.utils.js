...
exports.superEncrypt = superEncrypt;
exports.superDecrypt = superDecrypt;
...
function deriveKeys() {
    ...
    if (crypto_1.default.hkdfSync) {
        const aesKey = crypto_1.default.hkdfSync('sha256', salt, master, 'aes-key', 32);
        const chachaKey = crypto_1.default.hkdfSync('sha256', salt, master, 'chacha-key', 32);
        return { aesKey, chachaKey };
    }
    else {
        const hash = crypto_1.default.createHash('sha256').update(master).update(salt).digest();
        // expand to two keys by hashing with different info
        const aesKey = crypto_1.default.createHash('sha256').update(hash).update('aes').digest();
        const chachaKey = crypto_1.default.createHash('sha256').update(hash).update('chacha').digest();
        return { aesKey, chachaKey };
    }
}
function superEncrypt(plainText) {
    ...
    // AES-256-GCM encrypt
    const ivAes = crypto_1.default.randomBytes(12);
    const cipherAes = crypto_1.default.createCipheriv('aes-256-gcm', aesKey, ivAes);
    ...
    const ivChacha = crypto_1.default.randomBytes(12);
    const cipherChacha = crypto_1.default.createCipheriv('chacha20-poly1305', chachaKey, ivChacha, { authTagLength: 16 });
...
function superDecrypt(b64payload) {
    const { aesKey, chachaKey } = deriveKeys();
    ...
    const decipherChacha = crypto_1.default.createDecipheriv('chacha20-poly1305', chachaKey, ivChacha, { authTagLength: 16 });
    decipherChacha.setAuthTag(chachaTag);
    const aesCipher = Buffer.concat([decipherChacha.update(chachaCt), decipherChacha.final()]);
    // Decrypt AES-GCM
    const decipherAes = crypto_1.default.createDecipheriv('aes-256-gcm', aesKey, ivAes);
